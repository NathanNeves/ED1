public class Arvbin<T extends Comparable<T>>
{
	...
	private int calculaAlturadiametro() {
		if((esq == null) && (dir == null))
			return 0;
		...
		if(esq != null)
			altEsq = esq.calculaAlturadiametro();
		
		if(dir != null)
			altDir = dir.calculaAlturadiametro();
		
		int valor = altEsq + altDir+1; 
		this.dim = Math.max(this.dim,valor);
